...
export default function Page() {
  ...
  const singUp = async (e) => {
    ...
    try {
      ...
      if (!user.name || !user.phone || !user.email || user.name.length < 3 || user.phone.length < 10 || user.email.length < 5) {
      ...
      if (response.ok) {
        ...
        window.alert('Enter the otp sent to your mobile number');
      } else {
  ...
  return (
    <div className="h-screen flex">
      <div className=" w-3/5 flex items-center justify-center flex-col">
        <form onSubmit={singUp} method="POST" className="flex flex-col items-center justify-center gap-5">
          <div className="flex flex-col">
            <label htmlFor="" className="text-sm mb-1">Name</label>
            <input type="text" className="w-[20rem] p-3 border focus:outline-none rounded-xl" name="name" value={user.name} onChange={handleChanges} placeholder="Name" />
          </div>
          <div className="flex flex-col">
            <label htmlFor="" className="text-sm mb-1">Phone</label>
            <input type="text" className="w-[20rem] p-3 border focus:outline-none rounded-xl" name="phone" value={user.phone} onChange={handleChanges} placeholder="mobile" />
          </div>
          <div className="flex flex-col">
            <label htmlFor="" className="text-sm mb-1">Email</label>
            <input type="text" className="w-[20rem] p-3 border focus:outline-none rounded-xl" name="email" value={user.email} onChange={handleChanges} placeholder="email" />
          </div>
          <button className="bg-orange-500 px-20 py-3 text-white rounded-xl hover:bg-orange-600 w-full" type="submit">Submit</button>
        </form>
        {/* <form onSubmit={verifyOtp} method="POST" className="flex flex-col items-center justify-center gap-5">
          <div className="flex flex-col">
            <label htmlFor="">OTP</label>
            <input type="text" className="w-[20rem] p-2 border focus:outline-none" name="otp" value={otp} onChange={(e) => setOtp(e.target.value)} placeholder="otp" />
          </div>
          <button className="bg-orange-500 px-20 py-3 text-white rounded-xl hover:bg-orange-600 w-full" type="submit">Submit</button>
        </form> */}
        <div className="flex items-center justify-center gap-2 w-2/5 mt-5">
          <div className="border-b w-2/5"></div>
          <div className=" text-gray-400">Or</div>
          <div className="border-b w-2/5"></div>
        </div>
        <div className="flex flex-col gap-5 mt-10">
          <button type="button" className="bg-orange-100 px-20 py-3 text-orange-500 rounded-xl hover:bg-orange-200 flex items-center gap-2 w-full" onClick={() => signIn('google')}><FcGoogle size={20} />Login with google</button>
          <button type="button" className="bg-orange-100 px-20 py-3 text-orange-500 rounded-xl hover:bg-orange-200 flex items-center gap-2 w-full" onClick={() => signIn('facebook')}><FaFacebookF className="text-[#3b5998]" size={20} />Login with Facebook</button>
        </div>
      </div>
      <div className="bg-orange-100 h-screen w-2/5 flex items-center justify-center">
        <SignupImage />
      </div>
    </div>
  );
...
const SignupImage = () => (
  ...
  >
    ...
    <path
      d="M283.699 711.546L112.728 678.336L112.967 677.105L282.708 710.076L294.307 650.361L138.939 620.182L134.449 602.976L135.661 602.659L139.952 619.1L295.776 649.368L283.699 711.546Z"
      fill="#201F30"
    />
    <path
    ...
    />
    <path
      d="M342.098 587.252L120.921 544.29L120.682 545.522L341.859 588.484L342.098 587.252Z"
      fill="#201F30"
    />
    <path
    ...
    />
    <path
      d="M304.3 490.634L269.283 623.736L270.495 624.055L305.512 490.953L304.3 490.634Z"
      fill="#201F30"
    />
    <path
